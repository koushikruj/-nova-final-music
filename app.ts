...
export const app = express();
  // On Vercel, the body is automatically parsed by the serverless function handler.
  // Using express.json() can overwrite it or hang if the stream is consumed.
  app.use((req, res, next) => { if (req.body && Object.keys(req.body).length > 0) { return next(); } express.json()(req, res, next); });
